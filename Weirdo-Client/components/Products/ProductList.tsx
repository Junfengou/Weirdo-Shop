...
{/* md:grid-cols-2 sm:border-2 border-cyan-400 */}
      <div className='grid grid-cols-2 gap-4 content-normal ml-5 product-list-view-mobile-l:grid-cols-2 product-list-view-mobile-m:grid-cols-1 product-list-view-tablet-l:grid-cols-1'>
        {productListRecoilState?.products?.map(item => (
          <Link key={item?.id} href={`/products/${item?.id}`} className="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 sm:ml-6">
